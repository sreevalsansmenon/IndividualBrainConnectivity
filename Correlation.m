% MODIFIED FROM STEVE NETS NETMATS
...
%
% z: set to 1 to convert from r to z,
%    set to 0 to leave netmats as r,
%    set to <0 to pre-supply the (negative of the) r2z scaling factor to be applied
...
for s=1:size(ts,1)/NtimepointsPerSubject
  grot=ts((s-1)*NtimepointsPerSubject+1:s*NtimepointsPerSubject,:);
...
if (strcmp(method, 'multiggm'))
    ...
    NS = repmat(NtimepointsPerSubject,1,size(ts,2)/NtimepointsPerSubject);
    [grotPRECISIONS grotOBJ]= learn_multitask_ggm(reshape(netmats',N,N,size(ts,2)/NtimepointsPerSubject), NS, rho, 20);
    for s=1:size(ts,1)/NtimepointsPerSubject
...
if do_rtoz~=0 && MethodType>0

  if do_rtoz<0
    netmats=0.5*log((1+netmats)./(1-netmats))* (-do_rtoz);
  end
  ...
  if do_rtoz==1
    ...
    for s=1:size(ts,1)/NtimepointsPerSubject
      for i=1:N
        grot(1)=randn(1);
        for t=2:NtimepointsPerSubject
          grot(t)=grot(t-1)*arone+randn(1);
        end
        grotts((s-1)*NtimepointsPerSubject+1:s*NtimepointsPerSubject,i)=grot;
      end
      if MethodType==1
        grotr=corr(grotts((s-1)*NtimepointsPerSubject+1:s*NtimepointsPerSubject,:));
      else
        grotr=-inv(cov(grotts((s-1)*NtimepointsPerSubject+1:s*NtimepointsPerSubject,:)));
...
if ( INMODE == 1 )
  if size(netmats,1)~=1
    netmats=mean(netmats);
  end
  fc = reshape(netmats,N,N);
end
